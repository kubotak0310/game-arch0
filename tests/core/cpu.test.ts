...
describe('MOV 命令', () => {
  it('MOV R0, 2 で R0 に 2 が入る', () => {
    const result = execute('MOV R0, 2')
    expect(result.snapshot.registers.R0).toBe(2)
  })

  it('レジスタ間コピー MOV R1, R0', () => {
    const result = execute('MOV R0, 7\nMOV R1, R0')
...
describe('ADD 命令', () => {
  it('ADD R2, R0, R1 で R2 に R0+R1 が入る', () => {
    const result = execute(`
      MOV R0, 2
      MOV R1, 3
      ADD R2, R0, R1
    `)
    expect(result.snapshot.registers.R2).toBe(5)
  })

  it('ADD 即値形式: ADD R0, R0, 10', () => {
    const result = execute('MOV R0, 5\nADD R0, R0, 10')
    expect(result.snapshot.registers.R0).toBe(15)
  })

  it('16ビット折り返し: 0xFFFF + 1 = 0', () => {
    const result = execute('MOV R0, 0xFFFF\nADD R0, R0, 1')
    expect(result.snapshot.registers.R0).toBe(0)
  })
  ...
  it('ネガティブフラグ: bit15が1でN=true', () => {
    const result = execute('MOV R0, 0x7FFF\nADD R0, R0, 1')
...
describe('SUB 命令', () => {
  ...
  it('即値形式: SUB R0, R0, 3', () => {
    const result = execute('MOV R0, 10\nSUB R0, R0, 3')
    expect(result.snapshot.registers.R0).toBe(7)
  })

  it('ゼロフラグ: 3 - 3 = 0 でZ=true', () => {
    const result = execute('MOV R0, 3\nSUB R0, R0, 3')
    expect(result.snapshot.flags.Z).toBe(true)
  })
  ...
  it('16ビット折り返し: 0 - 1 = 0xFFFF', () => {
    const result = execute('MOV R0, 0\nSUB R0, R0, 1')
    expect(result.snapshot.registers.R0).toBe(0xFFFF)
  })
})
...
describe('HALT 命令', () => {
  it('HALT で実行が停止する', () => {
    ...
    expect(result.snapshot.registers.R0).toBe(0) // HALT後の命令は実行されない
  })
  ...
  it('HALT後に stepForward しても状態が変わらない', () => {
    ...
    expect(snap1.registers.R0).toBe(snap2.registers.R0)
  })
})
...
describe('ステップ実行と巻き戻し', () => {
  it('ステップ実行で巻き戻しができる', () => {
    ...
    cpu.load('MOV R0, 2\nMOV R1, 3')
    cpu.stepForward()
    expect(cpu.snapshot.registers.R0).toBe(2)
    cpu.stepBackward()
    expect(cpu.snapshot.registers.R0).toBe(0)
  })
  ...
  it('複数ステップの巻き戻し', () => {
    ...
    cpu.stepBackward()
    cpu.stepBackward()
    expect(cpu.snapshot.registers.R0).toBe(1)
  ...
  it('巻き戻し後の stepForward で履歴が分岐する', () => {
    const cpu = new Cpu()
    cpu.load('MOV R0, 1\nMOV R1, 2')
    cpu.stepForward() // R0 = 1
    cpu.stepBackward() // 巻き戻し
    cpu.stepForward() // 再び R0 = 1 (R2への分岐は破棄)
    expect(cpu.snapshot.registers.R0).toBe(1)
    expect(cpu.snapshot.registers.R1).toBe(0)
  })
})
...
describe('ExecutionResult の差分', () => {
  ...
  it('変化なしなら changedRegisters は空', () => {
    const cpu = new Cpu()
    cpu.load('MOV R0, 0') // R0 は初期値も 0 なので変化なし
    const result = cpu.stepForward()
...
describe('パースエラー', () => {
  ...
  it('パースエラーがあっても runAll は実行される（命令なし）', () => {
    ...
    expect(result.snapshot.registers.R0).toBe(0) // 命令なしなので変化なし
  })
})

describe('条件付き分岐 (BEQ/BNE/BLT/BGT/BLE/BGE)', () => {
  it('BNE: 等しければジャンプしない（PC は次へ進む）', () => {
    // R0=R1=5, BNE は飛ばず MOV R2, 1 が実行される
    const result = execute(`
      MOV R0, 5
      MOV R1, 5
      MOV R2, 0
    ...
    `)
    expect(result.snapshot.registers.R2).toBe(1)
  })

  it('BNE: 等しくなければジャンプする', () => {
    // R0=5, R1=3, BNE で MOV R2, 1 をスキップ
    const result = execute(`
      MOV R0, 5
      MOV R1, 3
      MOV R2, 0
    ...
    `)
    expect(result.snapshot.registers.R2).toBe(0)
  })
  ...
  it('JMP: 無条件ジャンプ', () => {
    const result = execute(`
      ...
      MOV R0, 1
    `)
    expect(result.snapshot.registers.R0).toBe(1)
  })
  ...
  it('ループ: 1から5までの累積', () => {
    ...
    const result = execute(`
      ...
      BNE loop
    `)
    expect(result.snapshot.registers.R0).toBe(15) // 1+2+3+4+5
  })
...
describe('initialRegisters / initialMemory', () => {
  it('initialRegisters を渡すと該当レジスタに初期値がセットされる', () => {
    const cpu = new Cpu()
    cpu.load('MOV R2, R0', undefined, undefined, { R0: 42, R1: 7 })
    expect(cpu.snapshot.registers.R0).toBe(42)
    expect(cpu.snapshot.registers.R1).toBe(7)
    cpu.runAll()
    expect(cpu.snapshot.registers.R2).toBe(42)
  })
...
describe('LOAD / STORE 命令', () => {
  ...
  it('LOAD with offset: [R1 + 4]', () => {
    const cpu = new Cpu()
    cpu.load('LOAD R0, [R1 + 4]', undefined, [{ address: 4, value: 123 }])
    cpu.runAll()
    expect(cpu.snapshot.registers.R0).toBe(123)
  })

  it('STORE with offset: [R1 + 2]', () => {
    const result = execute(`
      MOV R0, 7
      MOV R1, 8
      STORE R0, [R1 + 2]
...
describe('CALL / RET / PUSH / POP', () => {
  it('CALL は LR に戻り先を保存してジャンプする', () => {
    ...
    `)
    cpu.runAll()
    expect(cpu.snapshot.registers.R2).toBe(99) // sub が R0=99 にして RET で戻った
  })
  ...
  it('PUSH と POP でスタック経由の値の保存・復元', () => {
    const result = execute(`
      ...
      POP R1
    `)
    expect(result.snapshot.registers.R1).toBe(42)
  })
...
describe('SHL / SHR / 論理演算', () => {
  it('SHL: 左シフト', () => {
    const result = execute('MOV R0, 1\nSHL R1, R0, 3')
    expect(result.snapshot.registers.R1).toBe(8)
  })

  it('SHR: 右シフト', () => {
    const result = execute('MOV R0, 16\nSHR R1, R0, 2')
    expect(result.snapshot.registers.R1).toBe(4)
  })

  it('AND: ビット論理積', () => {
    const result = execute('MOV R0, 12\nMOV R1, 10\nAND R2, R0, R1')
    expect(result.snapshot.registers.R2).toBe(12 & 10) // 8
  })

  it('OR: ビット論理和', () => {
    const result = execute('MOV R0, 12\nMOV R1, 10\nOR R2, R0, R1')
    expect(result.snapshot.registers.R2).toBe(12 | 10) // 14
  })

  it('XOR: ビット排他的論理和', () => {
    const result = execute('MOV R0, 12\nMOV R1, 10\nXOR R2, R0, R1')
    expect(result.snapshot.registers.R2).toBe(12 ^ 10) // 6
  })
