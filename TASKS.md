...
**Phase A: ループ基礎**
- [x] S01: 累積加算 — カウンタダウンループの基本（`c3-s01-accumulate`、旧 c1-s07）
- [ ] S02: 配列の合計 — ループ + LOAD（Ch2-S07 の手動展開版を再訪）
- [ ] S03: 配列のコピー — ループ + LOAD/STORE（Ch2-S10 の再訪）

**Phase B: 符号付き分岐とループ応用**
- [ ] S04: 符号付き比較 — BLT/BGE 初出（単体問題、ループなし）
- [ ] S05: 配列の最大値 — ループ + 符号付き比較の総合

**Phase C: サブルーチン**
- [ ] S06: 関数を呼ぶ — CALL/RET 初出（引数なし・戻り値固定）
- [ ] S07: 引数と戻り値 — R1/R2 呼び出し規約
- [ ] S08: レジスタを保存 — PUSH/POP（callee-saved の必要性）
- [ ] S09: ネストした関数 — LR 保存
- [ ] S10: 章のまとめ — 関数を組み合わせて配列処理

**ストーリー作業**
- [ ] インタールード文章執筆（物語の転機・日付付きメモ・空白の日）
- [ ] 章内本文への独白の溶け込ませ（Ch2 と同様の方針）
