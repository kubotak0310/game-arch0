...
- 欄外メモはステージデータの `note.marginNote` フィールドに格納（既存型を流用）
- 章間インタールードは `note.interlude` フィールドに格納（既存型を流用）
- 終章メッセージは新規データとして別途定義（最終ステージのクリア後イベント）
- 「fade-in方式」を実現するため、ステージの章番号によって欄外メモの量・トーンを変える
- i18n は当面日本語のみ。英語版は将来対応（型構造は維持）
