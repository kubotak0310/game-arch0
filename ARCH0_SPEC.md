...
| 名前 | 特性 | 備考 |
| ---- | ---- | ---- |
| R0   | 汎用 | 呼び出し規約上は第1引数・戻り値（[3-4](#3-4-呼び出し規約calling-convention) 参照） |
| R1   | 汎用 | 呼び出し規約上は第2引数 |
| R2   | 汎用 | caller-saved スクラッチ |
...
| レジスタ | 種別 | 役割 |
| -------- | ---- | ---- |
| R0, R1   | Caller-saved（引数・戻り値） | R0=第1引数・戻り値、R1=第2引数 |
| R2       | Caller-saved（スクラッチ）   | 呼び出し元が必要なら CALL 前に保存する。関数は自由に使ってよい |
...
MOV  R0, 10        ; 第1引数
MOV  R1, 20        ; 第2引数
CALL add_r0_r1     ; 関数呼び出し（LR に戻りアドレスが入る）
; R0 に戻り値（30）が入っている
HALT
...
引数はレジスタ（R0, R1）で渡すため、CALL 命令自体は SP を変更しない。呼び出し先は RET 前に自分が使った SP を復元する義務を持つ。これにより関数本体で CALL を何度行っても SP は prologue 後の値を保つ。
...
[0x10]        ; 直接アドレス（数値リテラル）
[R0]          ; レジスタ間接
[R0 + 4]      ; オフセット付き（Intel風、数式的に読める）
```

**実装状況：** `[R0]`・`[R0 + 4]` 形式は第1章から実装済み。  
...
```asm
; 配列の合計を計算する例
MOV  R0, 0x10      ; 配列の先頭アドレス（ユーザーデータ領域 0x00〜0x3F 内）
MOV  R1, 8         ; 要素数
MOV  R2, 0         ; 合計の初期値
...
loop:
  CMP  R1, 0       ; R1 が0かチェック
  BEQ  end
  LOAD R3, [R0]    ; メモリから読み込み
  ADD  R2, R2, R3  ; 合計に加算
  ADD  R0, R0, 1   ; アドレスを進める
  SUB  R1, R1, 1   ; カウンタを減らす
  JMP  loop
